...
//
// @smoke — first of two specs in the starter's E2E auth suite. Tagged via
// the title prefix so CI can later filter with `--grep @smoke` once
// non-smoke specs exist.
...
// Until that's swapped for a real client-side `signOut()` call, the
// simplest reliable way to drop the session in E2E is to clear
