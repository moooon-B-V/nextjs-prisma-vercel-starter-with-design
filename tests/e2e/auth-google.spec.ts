...
//
// @smoke — second of two specs in the starter's E2E auth suite.
//
// What's mocked:
...
// Add a "set a password" flow on your profile-settings page if you want
// to support this.
...
const TEST_USER_PATH = process.env['E2E_TEST_OAUTH_USER_PATH'] ?? '/tmp/auth-test-oauth-user.json';
...
test('@smoke Google OAuth happy path + email-first auto-link', async ({ page }) => {
  ...
  //
  // This part exercises the supported auto-link direction wired in
  // lib/auth/index.ts: trustedProviders: ['google'] +
  // requireLocalEmailVerified: false. Better-Auth's
