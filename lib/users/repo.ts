...
//   2. findOrCreateOAuthUser — the auto-link gate. Better-Auth's
//      social-provider flow honors the same auto-link policy via its own
//      accountLinking config in lib/auth/index.ts; this helper exposes
//      the same semantics to direct-DB callers (tests, admin tooling).
//
 ...
 *
 * The default threat model accepts the Google-account-compromise →
 * local-account-takeover risk in branch (2) because Google has already
 * verified the email. If your application's threat model disagrees,
 * remove `'google'` from `account.accountLinking.trustedProviders` in
 * lib/auth/index.ts to disable auto-link.
 */
