...
//
// Google OAuth ships as a peer sign-in method. The auto-link policy lives in
// Better-Auth's `account.accountLinking` config (trustedProviders:
...
// of creating a duplicate. The security trade-off (Google-compromise →
// account takeover) is acceptable because Google has already verified the
// email; remove `'google'` from trustedProviders if your threat model
// disagrees.
//
// Provide your own Google Cloud OAuth credentials via env vars (see
// .env.example). No shared defaults ship with the starter. Missing creds →
// requiredEnv throws at module load, surfacing the gap loudly instead of
// letting the Google button error mysteriously at click time.
//
// Password reset is wired below via emailAndPassword.sendResetPassword.
// Better-Auth mounts the request endpoint at /api/auth/request-password-reset
// and the confirm endpoint at /api/auth/reset-password automatically; reset
// tokens are stored in the existing Verification table
// (identifier = "reset-password:<token>", value = userId). No
// PasswordResetToken table is needed — see the schema's Verification
// docstring for the wider rationale.
//
// Email verification stays off in this starter; wire requireEmailVerification
// on once you've designed the verification-email UX for your app.
...
export const auth = betterAuth({
  ...
  //      canonical public URL (e.g. https://your-app.vercel.app).
  //   2. VERCEL_BRANCH_URL — Vercel-injected, the stable branch-alias URL
  //      (e.g. your-app-git-<branch>-<team>.vercel.app). This is the
  ...
  emailAndPassword: {
    ...
    // 1 hour matches Better-Auth's default; pinned explicitly so a future
    // framework default change can't silently widen the reset-token window.
  ...
  // client IP (see @better-auth/core's get-request-ip.ts). The default rule
  // for /request-password-reset is approximated here as "3 requests per
  // hour per IP" — Better-Auth can't bind a rule to a body field, only to
  // a request path, so this is per-IP, not per-email. A single attacker
  // behind one IP can't enumerate; the small UX cost of shared-NAT users
  // hitting the limit is acceptable for v1. Note also that `enabled`
  // defaults to `true` only in production — we set it explicitly so the
  // limiter is active in dev and tests too.
  //
  ...
  account: {
    accountLinking: {
      ...
      // oauth2/link-account.mjs line 22). Email/password sign-up does NOT
      // set emailVerified=true (the verification UX isn't wired by default
      // in this starter), so leaving this default on would block the very
      // flow `trustedProviders: ['google']` was meant to enable: email-first
      ...
      // Account row with a password hash. Add a "set a password" flow in
      // your profile-settings surface if you want to support this.
    ...
    // pinned here for review-visibility.
    updateAccountOnSignIn: true,
  },

  // Better-Auth's default session cookie is already httpOnly + sameSite=lax
  // + secure-in-production. Pinning the explicit settings here keeps them
